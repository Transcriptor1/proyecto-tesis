...
if (isset($_POST['eliminar_id'])) {
    ...
    $stmt->execute();
    $stmt->close();
    header("Location: medios_registros.php?msg=" . rawurlencode("Registro eliminado") . "&tipo=success");
    exit;
}

$esAdmin = is_admin();
$etiquetasPorRegistro = etiquetas_por_registro($conn, 'medios');
?>
<!DOCTYPE html>
<html lang="es">
...
<body>
  ...
  <main>
    ...
    <?php render_page_actions('medios', 'registros'); ?>
      ...
      <div class="table-card">
        <table>
          ...
          while ($f = $r->fetch_assoc()) {
            ...
            echo "<a href=\"etiqueta_asignar.php?modulo=medios&id=" . (int) $f['id'] . "\" class=\"btn-edit\">Etiquetas</a></td>";
            if ($esAdmin) {
              echo "<td>"
                . "<a href=\"medios.php?id=" . (int) $f['id'] . "\" class=\"btn-edit\">Editar</a>"
